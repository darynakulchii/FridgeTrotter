...
async function loadCompanions() {
    ...
    try {
        // Запускаємо запит на отримання оголошень
        const adsPromise = fetch(`${API_URL}/companion/ads?${params}`, { headers: getHeaders() });

        // Якщо користувач залогінений, паралельно отримуємо список збережених, щоб підсвітити закладки
        let savedIds = new Set();
        if (currentUser) {
            try {
                const savedPromise = fetch(`${API_URL}/companion/saved`, { headers: getHeaders() });
                const [adsRes, savedRes] = await Promise.all([adsPromise, savedPromise]);

                if (savedRes.ok) {
                    const savedData = await savedRes.json();
                    if (savedData.ads) {
                        savedIds = new Set(savedData.ads.map(a => a.ad_id));
                    }
                }

                const data = await adsRes.json();
                renderAds(container, data.ads, savedIds);
            } catch (e) {
                console.warn('Error fetching saved status:', e);
                // Якщо не вдалося отримати збережені, просто рендеримо оголошення
                const response = await adsPromise; // (або перевикористовуємо результат якщо Promise.all впав)
                // Спрощуємо логіку помилок: якщо saved впав, просто показуємо ads
                const data = await (await fetch(`${API_URL}/companion/ads?${params}`, { headers: getHeaders() })).json();
                renderAds(container, data.ads, new Set());
            }
        } else {
            // Не залогінений
            const response = await adsPromise;
...
function renderAds(container, ads, savedIds) {
    container.innerHTML = '';
    ...
    ads.forEach(ad => {
        ...
        // Статус збереження
        const isSaved = savedIds.has(ad.ad_id);
        const bookmarkClass = isSaved ? 'fas' : 'far'; // fas = filled, far = outline
        const bookmarkBtnClass = isSaved ? 'text-[#48192E] border-[#48192E]' : '';

        const html = `
            <div class="universal-card cursor-pointer group" onclick="openAdDetails(${ad.ad_id})">
                
                <div class="card-header-user">
                    <a href="${profileLink}" class="card-avatar" style="background-color: #2D4952;" onclick="event.stopPropagation()">
                        ${avatarContent}
                    </a>
                    <div class="card-user-info">
                        <div class="flex items-center gap-2">
                            <a href="${profileLink}" class="card-user-name hover:underline" onclick="event.stopPropagation()">
                ...
                <div class="card-footer gap-2 px-4 py-3 border-t border-gray-100 flex items-center !mt-0">
                    <div class="mr-auto"></div> <button onclick="event.stopPropagation(); toggleSaveAd(${ad.ad_id}, event)" class="btn-icon-square ${bookmarkBtnClass}" title="Зберегти">
                        <i class="${bookmarkClass} fa-bookmark" id="ad-bookmark-${ad.ad_id}"></i>
                    </button>
                    <button onclick="event.stopPropagation(); openAdDetails(${ad.ad_id})" class="btn-outline px-4 text-sm h-10 w-auto">
                        Деталі
                    </button>
                    <button onclick="event.stopPropagation(); contactUser(${ad.user_id})" class="btn-fill px-4 text-sm h-10 w-auto">
                        Написати
                    </button>
                </div>
            </div>
...
window.toggleSaveAd = async (adId, event) => {
    ...
    try {
        ...
        if(res.ok) {
            // Перемикання класів іконок
            icon.classList.toggle('fas'); // Solid
            icon.classList.toggle('far'); // Outline

            // Перемикання стилю кнопки
            if (icon.classList.contains('fas')) {
                icon.parentElement.classList.add('text-[#48192E]', 'border-[#48192E]');
            } else {
                icon.parentElement.classList.remove('text-[#48192E]', 'border-[#48192E]');
            }
        }
