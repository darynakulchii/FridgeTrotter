...
// GET /api/companion/saved - Отримання збережених оголошень
router.get('/saved', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const query = `
        SELECT ca.*, up.first_name, up.last_name, up.profile_image_url AS author_avatar, usca.saved_date,
        (SELECT array_agg(t.tag_name) FROM companion_ad_tags cat JOIN tags t ON cat.tag_id = t.tag_id WHERE cat.ad_id = ca.ad_id) AS tags,
        COALESCE(ARRAY_AGG(cai.image_url) FILTER (WHERE cai.image_url IS NOT NULL), '{}') AS images
        FROM user_saved_companion_ads usca
        JOIN companion_ads ca ON usca.ad_id = ca.ad_id
        JOIN user_profiles up ON ca.user_id = up.user_id
        LEFT JOIN companion_ad_images cai ON ca.ad_id = cai.ad_id
        WHERE usca.user_id = $1
        GROUP BY ca.ad_id, up.user_id, usca.saved_date
        ORDER BY usca.saved_date DESC;
    `;
    try {
...
// POST /api/companion/ads/:id/save - Додати в обране
router.post('/ads/:id/save', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const adId = req.params.id;
    try {
        await pool.query(
            `INSERT INTO user_saved_companion_ads (user_id, ad_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`,
            [userId, adId]
        );
        res.json({ message: 'Оголошення збережено', saved: true });
    } catch (error) {
        res.status(500).json({ error: 'Server error' });
    }
});

// DELETE /api/companion/ads/:id/save - Видалити з обраного
router.delete('/ads/:id/save', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    const adId = req.params.id;
    try {
        await pool.query(
            `DELETE FROM user_saved_companion_ads WHERE user_id = $1 AND ad_id = $2`,
            [userId, adId]
        );
        res.json({ message: 'Видалено з обраного', saved: false });
    } catch (error) {
        res.status(500).json({ error: 'Server error' });
    }
});

// DELETE /api/companion/saved - Очистити все
router.delete('/saved', authenticateToken, async (req, res) => {
    const userId = req.user.userId;
    try {
        await pool.query('DELETE FROM user_saved_companion_ads WHERE user_id = $1', [userId]);
        res.json({ message: 'Всі збережені оголошення видалено.' });
    } catch (error) {
        res.status(500).json({ error: 'Server error' });
    }
});
